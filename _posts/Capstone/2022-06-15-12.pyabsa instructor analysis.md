...
원래 이번 포스팅에서 Trainer 클래스에 대해 정리하려고 했는데, Trainer에서 train4atepc 메소드를 호출하면 Instructor 클래스가 생성되고 훈련까지 진행됩니다. 그래서 이번 포스팅에서는 실제 모델 생성 및 훈련이 주된 내용이 될 것 같습니다.
...
train4atepc 메소드는 [PyABSA/pyabsa/core/apc/training/apc_trainer.py](https://github.com/yangheng95/PyABSA/blob/42aac7857fd6fb781184433f68063581ed1c3cc4/pyabsa/core/apc/training/apc_trainer.py#L566)에 정의돼 있는데, @retry라는 심볼이 붙어있습니다.
...
#1: 검증 데이터로더 또는 opt에 save_mode가 존재하는 경우 데이터로더, 모델을 제거한 뒤 저장 경로를 리턴한다.

#2: 데이터로더만 제거하고, gpu 캐시를 정리한 뒤 model, opt, tokenizer를 리턴한다.
