...
3. fodder의 2번째 인덱스 d와 foo의 2번째 인덱스 o를 비교합니다. d가 o보다 빠르므로 fodder < foo는 false
...
위 syntax도 title을 match 시킬 수 있겠죠?
...
[데이터베이스 9번째 포스트](https://cutewelshcorgi.github.io/9.from-er-models-to-relations/)에서 Null값을 잠깐 다뤘던 적이 있었는데, Null값은 다음과 같은 상황에서 사용됩니다.
...
- x IS NULL → TRUE
- X IS NOT NULL → FALSE (NULL이라는 column은 존재하지 않으므로)
