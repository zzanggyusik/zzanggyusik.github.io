...
<img src="https://user-images.githubusercontent.com/76269316/128621779-a544d663-6b2e-4758-8a3f-e113d22dc125.png" alt="image" style="zoom:67%;" />
...
| :----------------------------------------------------------: | :----------------------------------------------------------: |
| ![image](https://user-images.githubusercontent.com/76269316/128465134-63add2b8-dac4-4962-a88b-263f55f59186.png) | ![image](https://user-images.githubusercontent.com/76269316/128621892-2c0487d5-a6a3-4a1a-89da-806a81dd0249.png) |
...
OLS 기반의 선형 회귀와 릿지의 경우 회귀 계수가 유사한 반면, 라쏘는 전체적으로 회귀 계수 값이 작고 두 모델과 다른 형태를 보이고 있습니다.

<br>

이번에는 5개의 교차 검증 폴드 세트로 분할해 평균 [RMSLE](https://seominseok4834.github.io/machine%20learning/5.regression/#%EC%84%A0%ED%98%95-%ED%9A%8C%EA%B7%80-%EB%AA%A8%EB%8D%B8-%ED%95%99%EC%8A%B5%EC%98%88%EC%B8%A1%ED%8F%89%EA%B0%80)를 측정해 보겠습니다.

이를 위해 get_avg_rmse_cv()를 만들었습니다.



**get_avg_rmse_cv()**

교차 검증을 통해 평균 RMSE  측정
...
![image](https://user-images.githubusercontent.com/76269316/128589205-c999cc15-b479-4986-aae2-e72d2dc26ba6.png)

5개의 폴드 세트로 학습한 후 평가해도 여전히 라쏘 모델이 다른 모델에 비해 성능이 떨어집니다.

릿지와 라쏘의 모델에 alpha 하이퍼 파라미터를 변화시키면서 최적 값을 도출해 보겠습니다.

이를 위해 print_best_params()를 생성했습니다.
...
![image](https://user-images.githubusercontent.com/76269316/128589325-114a532d-311b-46b8-8cd1-ddd1a51ef69b.png)

최적 하이퍼 파라미터를 적용해 다시 학습/예측/평가를 수행해 보겠습니다.

<br>

```python
#최적화 alpha 값으로 학습 데이터로 학습, 테스트 데이터로 예측 및 평가 수행
lr_reg = LinearRegression()
lr_reg.fit(X_train, y_train)
...
![image](https://user-images.githubusercontent.com/76269316/128589398-ba2032f9-4d57-463a-8ecf-f73ffbded557.png)

alpha 값 최적화 후 예측 성능이 향상 됐습니다.

모델별 회귀 계수를 보면, 이번에는 라쏘 모델보다 선형 회귀와 릿지 모델의 회귀 계수가 높습니다.



<br>

데이터 세트를 좀 더 가공해 보도록 하겠습니다.

첫 번째로 피처 데이터 세트의 분포도가 왜곡돼 있는지를 확인해 보겠습니다.

사이파이 stat 모듈 skew() 함수를 사용해 컬럼 데이터 세트의 왜곡 정도를 추출할 수 있습니다.

일반적으로 skew() 함수의 반환 값이 1 이상인 경우 왜곡 정도가 높다고 판단합니다. (상황에 따라 편차가 있을 수도 있습니다)

저는 1 이상일 경우 해당 피처의 왜곡도를 완화하기 위해 로그 변환을 적용했습니다.

```python
from scipy.stats import skew

#object가 아닌 숫자형 피처 컬럼 index 추출
features_index = house_df.dtypes[house_df.dtypes != 'object'].index

#위에서 추출한 숫자형 피처들에 skew() 함수 적용
skew_features = house_df[features_index].apply(lambda x : skew(x))

#왜곡 정도가 1 이상인 컬럼만 추출해 로그 변환 적용
skew_features_top = skew_features[skew_features > 1]
print('로그 변환 적용 전\n', skew_features_top.sort_values(ascending=False))
house_df[skew_features_top.index] = np.log1p(house_df[skew_features_top.index])
```

![image](https://user-images.githubusercontent.com/76269316/128590835-98c0e554-0eab-4b3e-8314-fd6b019dd000.png)
...
![image](https://user-images.githubusercontent.com/76269316/128590874-47f96f3a-cf38-4ad0-a660-b96d22e3a638.png)

여전히 높은 왜곡 정도를 갖는 피처가 있지만, 더 이상 로그 변환을 진행해도 개선하기 어려워 그대로 사용하겠습니다.

house_df의 피처를 일부 로그 변환했으므로 다시 원-핫 인코딩을 적용한 house_df_ohe를 만든 뒤, 최적 alpha 값과 RMSE를 출력해 보겠습니다.
...
|                      피처 로그 변환 전                       |                      피처 로그 변환 후                       |
| :----------------------------------------------------------: | :----------------------------------------------------------: |
| ![image](https://user-images.githubusercontent.com/76269316/128589325-114a532d-311b-46b8-8cd1-ddd1a51ef69b.png) | ![image](https://user-images.githubusercontent.com/76269316/128591043-e8e801fe-3c7a-43cd-9bf4-77ba9925ceea.png) |
...
![image](https://user-images.githubusercontent.com/76269316/128591158-a072d648-90aa-4b75-970b-825355533c14.png)



세 피처 모두 GrLivArea(주거 공간 크기)가 회귀 계수가 가장 높은 피처가 됐습니다.

주거 공간의 크기가 주택 가격에 미치는 영향이 제일 높다는 의미로, 이제야 상식선에서의 결과가 도출 됐습니다.

<br>

두 번째로, 예측에 많은 영향을 미치는 중요 피처 그 중에서 세 개 모델 모두에서 가장 큰 회귀 계수를 갖는 GrLivArea 피처의 이상치(Outlier) 데이터를 처리해 보겠습니다.

이를 위해 먼저 GrLivArea 피처와 타깃 값(SalePrice)의 관계를 시각화해 보겠습니다.
...
![image](https://user-images.githubusercontent.com/76269316/128591553-50615842-a4d5-4273-8bcd-439ace8cd3fc.png)

일반적으로 주거 공간이 큰 집일수록 가격이 비싸기 때문에 GrLivArea 피처는 양의 상관도가 매우 높음을 직관적으로 알 수 있습니다.

하지만 위 그림에서 네모로 표시한 2개의 데이터는 가장 큰 데이터임에도 불구하고 일반적인 GrLivArea와 SalePrice 관계에서 많이 어긋나 있습니다.

위 두 데이터를 이상치 데이터로 간주하고 삭제하겠습니다.

```python
#GrLivArea와 SalePrice 모두 로그 변환 됐으므로 이를 반영한 조건 생성
...
#DataFrame 인덱스를 이용해 이상치 레코드 삭제
house_df_ohe.drop(outlier_index, axis=0, inplace=True)
print('이상치 삭제 후 house_df_ohe shape: ', house_df_ohe.shape)
```

![image](https://user-images.githubusercontent.com/76269316/128591696-b4a098ed-3a80-4c51-8728-740ce70363b6.png)

업데이트 된 house_df_ohe를 기반으로 피처 데이터 세트와 타깃 데이터 세트를 다시 생성해 릿지와 라쏘 모델 최적화를 수행하고 결과를 출력해 보겠습니다.
...
![image](https://user-images.githubusercontent.com/76269316/128591743-f339a702-bc09-4938-b69b-5d49bf0d9c7b.png)

최적 하이퍼 파라미터를 적용해 학습/예측/평가를 수행해 보겠습니다.
...
![image](https://user-images.githubusercontent.com/76269316/128591787-609273a1-06c9-4b7d-a9bf-a4ac091c1538.png)

두 개의 이상치 데이터만 제거했는데도 예측 수치가 매우 크게 향상됐습니다.

일단 대략의 데이터 가공과 모델 최적화를 수행한 뒤, 다시 이에 기반한 여러 기법의 데이터 가공과 하이퍼 파라미터 기반의 모델 최적화를 반복적으로 수행하는 것이 바람직한 머신러닝 모델 생성 과정입니다.
...
![image](https://user-images.githubusercontent.com/76269316/128592328-ce27fe80-04e8-442d-bfb6-74f5290d67f8.png)
...
![image](https://user-images.githubusercontent.com/76269316/128592408-0c69e9a3-9170-44fc-85d0-dfa719bd3ac3.png)



각각의 트리 모델의 중요도를 시각화해 보겠습니다.

피처 개수가 많으므로 상위 20개의 피처명과 그 때의 중요도를 반환하는 함수 **get_top_features()**와 피처 중요도를 시각화 해주는 **visualize_ftr_importances()** 함수를 사용했습니다.



**get_top_features()**

상위 20개의 피처 중요도를 반환해주는 함수

```python
#모델의 중요도 상위 20개 피처명과 중요도값을 Series로 반환
...
**visualize_ftr_importances()**

```python
def visualize_ftr_importances(models):
    #2개 회귀 모델의 시각화를 위해 2개의 컬럼을 갖는 subplot 생성
    fig, axs = plt.subplots(figsize=(24, 10), nrows=1, ncols=2)
    fig.tight_layout()  #서브플롯 간 간격 자동 유지
    
    #입력 인자로 받은 list 객체인 models에서 차례로 model을 추출해 피처 중요도 시각화
    for i_num, model in enumerate(models):
        #중요도 상위 20개의 피처명과 중요도값 추출
        ftr_top20 = get_top_features(model)
        axs[i_num].set_title(model.__class__.__name + 'Feature Importances', size=25)
        #font 크기 조정
        for label in (axs[i_num].get_xticklabels() + axs[i_num].getyticklabels()):
            label.set_fontsize(22)
...
xgb_reg = XGBRegressor(n_estimators=1000, learning_rate=0.05, colsample_bytree=0.5, subsample=0.8)
best_xgb = xgb_reg.fit(X_train, y_train)

lgbm_reg = LGBMRegressor(n_estimators=1000, learning_rate=0.05, num_leaves=4, subsample=0.6, colsample_bytree=0.4, reg_lambda=10, n_jobs=-1)
best_lgbm = lgbm_reg.fit(X_train, y_train)

models = [best_xgb, best_lgbm]
visualize_ftr_importances(models)
```

![image](https://user-images.githubusercontent.com/76269316/128621368-c7b75952-3ddd-443f-bfd4-0ce9a566167a.png)
...
개별 회귀 모델의 예측 결괏값을 혼합해 이를 기반으로 최종 회귀 값을 예측하겠습니다.

예를 들어 A 회귀 모델의 예측값이 [100. 80. 60]이고 B 회귀 모델의 예측값이 [120, 80. 50]일 때,

A 모델 예측값이 40%, B 모델 예측값의 60%를 더해 최종 회귀 값으로 예측한다면 최종 회귀 예측값은 [100\*0.4 + 120\*0.6, 80\*0.4 + 80\*0.6, 60\*0.4 + 50\*0.6] = [112, 80. 54]가 됩니다. 

위에서 구한 릿지 모델(40%)과 라쏘 모델(60%)을 혼합해 보겠습니다.
...
![image](https://user-images.githubusercontent.com/76269316/128596765-cb46f0fb-814a-4132-8ec4-5a76960b1495.png)

개별 모델보다 성능이 약간 개선된 것을 확인할 수 있습니다.

<br>

이번에는 XGBoost(50%)와 LightGBM(50%)을 혼합해 보겠습니다.
...
마찬가지로 개별 모델의 RMSE보다 조금 향상 됐습니다.

![image](https://user-images.githubusercontent.com/76269316/128597050-f732c3ff-58f3-4c93-b6c3-76db7a7aebfa.png)
...
[분류에서 소개한 스태킹 모델](https://seominseok4834.github.io/machine%20learning/4.classification/#%EC%8A%A4%ED%83%9C%ED%82%B9-%EC%95%99%EC%83%81%EB%B8%94)을 회귀에도 적용할 수 있습니다.

분류와 개념이 동일하니 간단하게만 설명하면 스태킹은 개별 알고리즘의 예측 결과 데이터 세트를 최종적인 메타 데이터 세트로 만들어 별도의 ML 알고리즘으로 최종 학습을 수행하고 테스트 데이터를 기반으로 다시 최종 예측을 수행하는 방식입니다.

스태킹 모델의 핵심은 여러 개별 모델의 예측 데이터를 각각 스태킹 형태로 결합해 최종 메타 모델의 학습용 피처 데이터 세트와 테스트용 피처 데이터 세트를 만드는 것입니다.

분류에서도 사용했던 get_stacking_base_datasets() 함수를 사용해 회귀 예측을 해보겠습니다. 

<br>

**get_stacking_base_datasets**

개별 모델의 Regressor 객체, 원본인 학습용 피처 데이터, 원본 학습용 레이블 데이터, 원본 테스트 피처 데이터, K 폴드를 몇 개로 할지를 파라미터로 입력 받아 메타 모델을 위한 학습용 데이터와 테스트용 데이터를 반환
...
get_stacking_base_datasets()을 개별 모델(릿지, 라쏘, XGBoost, LightGBM)별로 적용해 메타 모델이 사용할 학습 피처 데이터 세트와 테스트 데이터 피처 데이터 세트를 추출하겠습니다.

```python
#get_stacking_base_datasets()는 넘파이 ndarray를 사용하므로 DataFrame을 넘파이로 변환
...
![image](https://user-images.githubusercontent.com/76269316/128597673-d31def94-b68d-4ad9-b930-7db5fd990f00.png)

각 개별 모델이 반환하는 학습용 피처 데이터와 테스트용 피처 데이터 세트를 결합해 최종 메타 모델에 적용해 보겠습니다.

메타 모델은 별도의 라쏘 모델을 사용하며, 최종적으로 예측 및 RMSE를 측정합니다.
...
![image](https://user-images.githubusercontent.com/76269316/128598546-0a3cdcf4-92f1-4a22-ba34-34a38ce82b59.png)

약 0.0975로 현재까지 가장 좋은 성능 평가를 보여줍니다.

스태킹 모델은 분류뿐만 아니라 회귀에서도 효과적으로 사용될 수 있는 모델입니다.
...
선형 회귀는 실제값과 예측값의 차이인 오류를 최소로 줄일 수 있는 선형 함수를 찾아 이 선형 함수에 독립변수(피처)를 입력해 종속변수(타깃값, 예측값)를 예측하는 것입니다.

최적의 선형 함수를 찾기 위해 실제값과 예측값 차이의 제곱을 회귀 계수 W를 변수로 하는 비용 함수로 만들고, 이 비용 함수가 최소화되는 W 값을 찾아 선형 함수를 도출할 수 있습니다.



실제값과 예측값의 차이를 최소화하는 것에만 초점을 맞추면 학습 데이터에 과적합되는 문제가 발생할 가능성이 높습니다.

이를 해결하기 위해 규제(Regularization)를 선형 회귀에 도입했습니다.

L2 규제를 적용한 릿지(Ridge), L1 규제를 적용한 라쏘(Lasso), L1에 L2 규제를 결합한 엘라스틱넷(ElasticNet)이 있습니다.

일반적으로 선형 회귀는 규제 선형 회귀를 많이 사용합니다.



선형 회귀를 분류에 적용한 대표적인 모델이 로지스틱 회귀(Logistic Regression)입니다. (이름은 회귀이지만 분류를 위한 알고리즘입니다)

선형 함수 대신 최적의 시그모이드 함수를 도출하고, 독립변수(피처)를 이 시그모이드 함수에 입력해 반환된 결과를 확률값으로 변환해 예측 레이블을 결정합니다.

로지스틱 회귀는 이진 분류나 희소 행렬로 표현되는 텍스트 기반 분류에서 높은 예측 성능을 나타냅니다.



회귀 트리를 이용해 예측하는 방법도 있습니다.

회귀 트리는 분류를 위해 만들어진 분류 트리와 크게 다르지 않으나, 회귀 트리는 리프 노드에 속한 데이터 값의 평균값을 구해 회귀 예측값을 계산한다는 점에서 분류 트리와 다릅니다.

분류와 동일하게 결정 트리, 랜덤 포레스트, GBM, XGBoost, LightGBM 모두 회귀 트리를 수행하는 방법을 제공합니다.



선형 모델을 기반으로 하는 선형 회귀는 데이터 값의 분포도와 인코딩 방법에 많은 영향을 받습니다.

선형 회귀는 정규 분포를 선호하며, 특히 타깃값의 분포도가 왜곡(Skew)되지 않고 정규 분포 형태로 되어야 예측 성능을 저하시키지 않습니다.

일반적으로 로그 변환을 적용하는 것이 유용합니다.

또한 선형 회귀의 경우  카테고리형 데이터가 있을 경우 이를 레이블 인코딩을 통한 숫자형 변환보다는 원-핫 인코딩으로 변환하는 것이 좋습니다.

회귀 트리의 경우 인코딩 방식에 크게 영향을 받지는 않습니다.
