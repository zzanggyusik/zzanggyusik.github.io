...
X_features와 y_label 데이터 세트를 기반으로 별다른 제약 없이(하이퍼 파라미터를 디폴트로) 결정트리로 학습한 뒤, 결정 트리 모델이 어떠한 결정 기준을 갖고 분할하면서 데이터를 분류하는지 색상과 경계로 나타내 확인해 보겠습니다.
...
실행 결과 최고 평균 정확도가 약 85.13%가 도출됐습니다.
